...
LogMessageCallback g_log_message_callback;

}  // namespace

void SetLogMessageCallback(LogMessageCallback callback) {
  g_log_message_callback = callback;
}
...
LogMessage::~LogMessage() {
  ...
  __android_log_write(priority, "clay", stream_.str().c_str());
#elif defined(OS_IOS) || defined(OS_TVOS)
  ...
  if (g_log_message_callback) {
    g_log_message_callback(GetNameForLogSeverity(severity_), file_, line_,
                           stream_.str().c_str());
  } else {
